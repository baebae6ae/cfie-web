...
async function loadChart(ticker) {
  ...
  try {
    ...
    renderFIS(computeFIS(data), data);
...
function renderChart(ticker, data) {
  ...
  _chart.addLineSeries({ color: "#0047AB", lineWidth: 1, title: "전환선" })
        .setData(ichi.tenkan.filter(d => d.value != null));
  _chart.addLineSeries({ color: "#CC0000", lineWidth: 1, title: "기준선" })
        .setData(ichi.kijun.filter(d => d.value != null));
  ...
  _volChart.addHistogramSeries({ color: "#bbb", priceFormat: { type: "volume" } })
           .setData(data.map(d => ({ time: d.time, value: d.volume, color: d.close >= d.open ? "#CC0000" : "#0047AB" })));
  _chart.timeScale().subscribeVisibleLogicalRangeChange(r => _volChart.timeScale().setVisibleLogicalRange(r));
...
function renderFIS(fis, data) {
  ...
  document.getElementById("fisPanel").classList.remove("hidden");
  ...
  document.getElementById("fpBars").innerHTML = [
